...
public class NewsController {
    ...
    @GetMapping("/top3")
    public ResponseEntity<Map<String, List<NewsResponse>>> getTop3NewsByCategories() {
        Map<String, List<NewsResponse>> newsByCategory = new HashMap<>();

        // 고정된 카테고리 목록
        String[] categories = {"celeb", "brand"};

        for (String category : categories) {
            List<NewsResponse> top3News = newsService.getTop3NewsByCategory(category);
